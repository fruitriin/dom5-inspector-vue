  ...
  <div>
    ...
    <div class="filter-field">
      ...
      <table>
        ...
        <tr>
          ...
          <td>
            <template v-for="element in elements">
              <input :id="'filter_' + element" type="checkbox" v-model="filters.elements" :value="element">
        ...
        <tr>
          <td>属性レベル</td>
          <td><input type="number" v-model="filters.elementLevelMin">
            〜<input type="number" v-model="filters.elementLevelMax"></td>
        </tr>
        <tr>
          <td>領域</td>
          <td>
            <template>
              <template v-for="(school, k) in schools">
                <input :id="'filter_' + school" type="checkbox" v-model="filters.schools" :value="school">
        ...
        <tr>
          <td>領域レベル</td>
          <td><input type="number" v-model="filters.schoolsLevelMin">
            〜<input type="number" v-model="filters.schoolsLevelMax"></td>
        </tr>
      </table>
...
<script>
  ...
  export default {
    ...
    data: function () {
      return {
        rawData: {},
        header: ["名称", "領域", "主系統", "複系統", "消費", "使用場面", "簡易分類"],
      ...
      data: function () {
        return Object.values(this.rawData).filter((d) => {
            ...
            if (this.filters.elements.length) {
              let filterdElements = _.difference(this.elements, this.filters.elements)
              if (filterdElements.length > 0) {
                ...
                if (!(filterdElements.indexOf(elements[mainElement]) === -1
